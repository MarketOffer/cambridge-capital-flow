    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        window.dispatchEvent(
          new CustomEvent("hero-cta-visibility", { detail: entry.isIntersecting })
        );
      },
      { threshold: 0 }
    );
        ...
        <div className="max-w-xl">
          <FadeIn>
            <h1 className="text-[2.5rem] font-semibold leading-[1.1] tracking-[-0.02em] text-foreground sm:text-5xl md:text-[3.25rem] lg:text-[4rem] xl:text-[4.5rem]">
              Investing in property for <span className="whitespace-nowrap">the AI era</span>
            </h1>
          </FadeIn>
          <FadeIn delay={0.15}>
            <p className="mt-8 max-w-md font-sans text-base leading-relaxed text-muted-foreground md:text-[1.0625rem]">
              We acquire residential property in cities at the centre of the AI transformation and occasionally partner with experienced business owners as investors on selected projects.
            </p>
          </FadeIn>
          <FadeIn delay={0.3}>
            <a
